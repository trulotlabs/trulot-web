...
// Find the richest scope description: if primary is vague, scan siblings
...
export async function GET(_req: Request, { params }: { params: Promise<{ apn: string }> }) {
  ...
  // ── Scope ───────────────────────────────────────────────────────────────────
  const scopeResult = getBestScope(
    (pp?.primary_project_description as string) ?? null,
    permits
  );

  // Find the richer ADU scope permit
  const aduScopePmt = permits.find(
    (p2) =>
      typeof p2.description === "string" &&
      /ADU|26.*unit|26.*adu/i.test(p2.description)
  );
  ...
  const ownerSignals = [
    ...(p.absentee_owner === true ? [{ key: "absentee_owner", value: "Yes", confidence: "source-backed" as ConfidenceLevel }] : []),
  ];
  ...
  const yearDisplay = yearRaw && yearRaw !== "00"
  ...
  const constraints = {
    ...
    regulatory: {
      fire_hazard: {
        ...
        note: hasVhfhsz ? "VHFSZ referenced in permit description — verify against mapped fire overlay" : undefined,
      },
  ...
  const result: ParcelPageData & { development_stage: string; jobs_to_engage?: unknown[] } = {
    ...
    readout: {
      ...
      signals: [
        ...(stage === "ACTIVE" || stage === "SCALING"
          ? [{ key: "active_construction", value: "Active construction", confidence: "inferred" as ConfidenceLevel }]
          : []),
        ...(proposedProject
          ? [{ key: "related_adu_scope", value: `${proposedProject.scope} on related permit`, confidence: "conditional" as ConfidenceLevel }]
          : []),
    ...
    project: {
      primary_permit: primaryPermit ?? {
        ...
        type: "None",
        status: "IN REVIEW",
        scope: "No building permit on file",
        confidence: "source-backed",
      },
    ...
    capacity: {
      baseline_units: {
        units: baselineUnits,
        basis: `${basisLine} + standard ADU allowances`,
        confidence: "source-backed",
        source: "SDMC",
      },
    ...
    context: {
      nearby_development: {
        ...
        nearest_completed: p.nearest_completed_distance_ft
          ? `${Math.round(p.nearest_completed_distance_ft as number)} ft`
          : "Unknown",
